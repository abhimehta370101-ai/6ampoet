...
// Lazily construct the client: the `resend` package throws synchronously if the
// API key is missing, and doing that at require-time would crash the whole
// process on boot before the server can even bind to a port.
function getClient() {
  if (!resend) {
